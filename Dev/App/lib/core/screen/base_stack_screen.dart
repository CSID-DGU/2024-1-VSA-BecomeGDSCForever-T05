...
import 'package:modeul/app/config/color_system.dart';

@immutable
abstract class BaseStackScreen<T extends GetxController> extends GetView<T> {
  const BaseStackScreen({super.key});
    ...
    return Material(
      child: Stack(
        children: [
          ...
          buildGreyScaleLayer(context),
          buildButtonAndManeLayer(context),
        ],
      ),
    );
  }
  ...
  /// 메뉴가 활성화될 때, 중간 Layer를 구성하는 메서드
  @protected
  Widget buildGreyScaleLayer(BuildContext context);

  /// 최상단 Layer의 버튼과 메뉴를 구성하는 메서드
  @protected
  Widget buildButtonAndManeLayer(BuildContext context);
}
